...
void main() {
  ...
  test('a sale with a chosen payment method books that method in Odoo', () async {
    ...
    final booked = ((await wiring.catalogueCall('pos.order', 'search_read',
        [[['uuid', '=', order.uuid]],
         ['id', 'amount_total', 'state', 'amount_paid']], {})) as List);
    print('ORDER: $booked');
    expect(booked.isNotEmpty, isTrue);
    final o = booked.first as Map;
    expect(o['state'], isNot('draft'),
        reason: 'the tendered sale should leave the POS order settled');
    expect(((o['amount_paid'] as num) - price).abs() < 0.05, isTrue,
        reason: 'paid amount should match the tender');
    db.close();
