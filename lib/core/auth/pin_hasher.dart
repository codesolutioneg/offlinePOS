...
/// Turns a PIN into something safe to store.
///
/// An interface rather than a single class for two reasons: the KDF is deliberately
/// slow, so anything not testing the KDF should not pay for it, and a future
/// parameter change must not mean touching call sites.
abstract interface class PinHasher {
  Future<String> hash(String pin, String saltB64);
  Future<bool> verify(String pin, String saltB64, String expectedB64);
...
class Argon2idPinHasher implements PinHasher {
  Argon2idPinHasher({
  ...
  Future<bool> verify(String pin, String saltB64, String expectedB64) async {
    ...
    try {
      expected = base64Decode(expectedB64);
    } catch (_) {
      return false;
    }
